...
/*
 ...
 -정적변수는 JVM(자바가상머신)에 의해 프로그램이 시작되는
 	시점에 초기화된다
 -main() 메소드가 실행되기 전에 이미 초기화듸므로 main()
...
class MyStatic {
	...
	/*
	 ...
	 프로그램 시작 시 이미 메소드영역에 로드된다.
	 */
	
	static int staticVar;
	
	/*
	 인스턴스헝 멤버메소드] 인스턴스형 메소드에서는 모든 멤버를
	 	사용할 수 있다. 인스턴스형 변수, 정적변수 둘다 접근 가능하다.
	 */
	void instanceMethod() {
	...
	/*
	 정적 멤버메소드] 정적메소드 안에서는 인스턴스 멤버를 사용할 수 없다.
	 	정적 멤버만 접근이 가능하다.(※main메소드에서는 static으로 선언된 메소드만 
	 	호출할 수 있었음을 기억하자.)
	 */
...
public class E01StaticModifier {
	...
	public static void main(String[] args) {
		...
		System.out.println("MyStatic.staticVar="+ MyStatic.staticVar);
		/*
		 	방법2: 일반적인 멤버변수처럼 참조변수를 통해 접근하는 것도
		 	가능하다. 단, 이경우는 정적변수를 일반적인 멤버변수로
		 	착각 할 수 있으므로 자바에서는 권장하지 않는다.
		 */
