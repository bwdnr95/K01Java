...
/*
 ...
 -Java에서는 main메소드는 무조건 public static void로
 	선언하기로 약속되어 있다.
-static으로 선언된 메소드는 static로 선언된 메소드만
	호출할 수 있다.
	-메소드 호출 후 반환값은 항상 호출된 위치로 반환된다. 이때
		메소드는 메모리에서 소멸된다.
-반환값이 없는 void형 메소드도
 실행이 완료되면 호출한 위치로 실행의 흐름이 돌아온다.
 */
public class E01MethodBasic {
	
	/*
	 메소드정의1 : 반환형O, 매개변수 O 형태로 정의
	...
	public static void main(String[] args) {
		/*
		 메소드를 호ㅜㄹ할때는 함수의 원형을 그대로 기술하면된다.
		 매개변수가 있다면 갯수만큼 포함시켜주면 된다. 매개변수의
		 갯수가 다르면 호출되지 않고 에러가 발생한다. 또는 매개변수의
		 타입이 맞지 않아도 에러가 발생한다.
		 */
