...
class Car {
	...
	*/
	...
	/*
	 ...
	 초기화 된다는 한계를 가지고 있다. 따라서 이와 같은 형태로는
	 만들지 않는 것이 좋다.
	 */
	void initalize() {
		carModel = "람보르기니";
	...
	/*
	 ...
	 객체를 초기화 할 수 있다.
	 */
	void initalize(String model, int year, String name, int age, int energy) {
		carModel = model;
		carYear = year;
		/*
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 초기화 되지 않는 상태에서 출력을 시도하여 런타임 에러가
		 발생된다.
		 */
		System.out.println("[초기화메소드 호출후]");
		car1.initalize();//항상 고정된 내용으로만 초기화 진행됨.
		car1.drive();
		car1.showCarInfo();
		
		//자동차2 객체 생성 : 객체 생성 후 멤버 변수에 직접 접근하여 초기화 한다.
		/*
		 만약 2개 이상의 객체를 생성해야 한다면 아래의 초기화 코드가 중복사용 되어야 하므로 
		...
		//세번째 자동차 객체 생성 : 매개변수가 있는 초기화 메소드를 이요하여 초기화 진행함.
